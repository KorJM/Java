...
public class CoinEatingGame {
    ...
    private Color background = new Color(237, 235, 233);
	...
	public void go() {
		...
		while (true) {		
			...
			if ((int)time / 40 == speedUp && time > 0) speedUp++;
			...
			if(hitGodMod || itemGodMod) {
				playerX += stepX * speedUp * 2 ;
				playerY += stepY * speedUp * 2;
			}else {
				playerX += stepX * speedUp;
				playerY += stepY * speedUp;
			}	
	...
	class GamePanel extends JPanel {
		public void paintComponent(Graphics g) {	
			...
			g.drawString("Level : " + speedUp, 30, 40);
			
			g.setColor(Color.BLACK);
			g.setFont(new Font("Arial", Font.BOLD, 40));
			g.drawString("진행 시간 : " + (int) time + "초", 30, 90);

			g.setColor(Color.BLACK);
			g.setFont(new Font("Arial", Font.BOLD, 40));
			g.drawString("SCORE : " + score, 30, 140);
			
			for(int i=0; i<lifeCnt; i++) {
				g.drawImage(life, 30 + i * 55 , 160, lifeWidth, lifeHeight, this);
			}
	...
	class Trap {
		...
		private void moveTraps() {
			for(Trap t: traps) {
				if(t.getX() <= 0) t.setStepX((int)(Math.random() * 3) * speedUp);
				else if(t.getX() >= WIDTH - 50) t.setStepX((int)(Math.random() * 3) * -1 * speedUp);
				if(t.getY() <= 0) t.setStepY((int)(Math.random() * 3) * speedUp);
				else if(t.getY() >= HEIGHT - 100) t.setStepY((int)(Math.random() * 3) * -1 * speedUp);
