...
public class CoinEatingGame {
	...
	private final float Item_God_Time = 3.0f;
	private float itemgodtime = Item_God_Time;
	private boolean hitGodMod = false;
	private boolean itemGodMod = false;
	private boolean itemDrop = false, healDrop = false, itemOn = false, itemRe = false, healRe = false;
	...
	private void pointCrashCheck() {
		if ((playerX + 30 >= pointX - 20 && playerX + 30 <= (pointX + pointWidth + 20))
			...
			pointY = (int) (Math.random() * (HEIGHT - playerHeight - 30)) + 20;
		}
	}
	
	private void itemCrashCheck() {
		if (score % 700 == 0 && score > 0 && !itemOn && !itemRe) {
			item_GodModX = (int)(Math.random() * (WIDTH - item_GodModWidth * 2) - item_GodModWidth);
			item_GodModY = (int)(Math.random() * (HEIGHT - item_GodModHeight * 2) - item_GodModHeight);
			itemDrop = true;
			itemOn = true;
		}
		...
		if (itemScore >= 2000 && lifeCnt < 3 && !healRe) {
			healDrop = true;
			healRe = true;
			item_HealX = (int)(Math.random() * (WIDTH - item_HealWidth * 2) - item_HealWidth);
			item_HealY = (int)(Math.random() * (HEIGHT - item_HealHeight * 2) - item_HealHeight);
			itemScore = 0;
		}
